...
static const u16 sMoveCategories_Pal[] = INCBIN_U16("graphics/interface/move_categories.gbapal");
static const u32 sMoveCategories_Gfx[] = INCBIN_U32("graphics/interface/move_categories.4bpp.lz");

static const struct OamData sOamData_MoveCategories =
{
...
static const struct CompressedSpriteSheet sSpriteSheet_MoveCategories =
{
    .data = sMoveCategories_Gfx,
    .size = 16*16*3/2,
    .tag = TAG_MOVE_CATEGORIES,
};

static const struct SpritePalette sSpritePal_MoveCategories =
{
    .data = sMoveCategories_Pal,
    .tag = TAG_MOVE_CATEGORIES
};
...
static const union AnimCmd *const sSpriteAnimTable_MoveCategories[] =
{
...
static const struct SpriteTemplate sSpriteTemplate_MoveCategories =
{
    .tileTag = TAG_MOVE_CATEGORIES,
    .paletteTag = TAG_MOVE_CATEGORIES,
    .oam = &sOamData_MoveCategories,
    .anims = sSpriteAnimTable_MoveCategories,
    .images = NULL,
...
// code
static u8 ShowMoveCategory(u32 moveCategory)
{
    if (sMonSummaryScreen->moveCategoriesSpriteId == 0xFF)
        sMonSummaryScreen->moveCategoriesSpriteId = CreateSprite(&sSpriteTemplate_MoveCategories, 48, 129, 0);

    gSprites[sMonSummaryScreen->moveCategoriesSpriteId].invisible = FALSE;
    StartSpriteAnim(&gSprites[sMonSummaryScreen->moveCategoriesSpriteId], moveCategory);
    return sMonSummaryScreen->moveCategoriesSpriteId;
}

static void DestroyMoveCategory(void)
{
    if (sMonSummaryScreen->moveCategoriesSpriteId != 0xFF)
        DestroySprite(&gSprites[sMonSummaryScreen->moveCategoriesSpriteId]);
    sMonSummaryScreen->moveCategoriesSpriteId = 0xFF;
}
...
static bool8 DecompressGraphics(void)
{
    ...
    case 12:
        ...
        LoadCompressedSpriteSheet(&sSpriteSheet_MoveCategories);
        LoadSpritePalette(&sSpritePal_MoveCategories);
        sMonSummaryScreen->switchCounter = 0;
        return TRUE;
    }
    return FALSE;
...
static void ShowCantForgetHMsWindow(u8 taskId)
{
    ClearWindowTilemap(PSS_LABEL_WINDOW_MOVES_POWER_ACC);
    ClearWindowTilemap(PSS_LABEL_WINDOW_MOVES_APPEAL_JAM);
    gSprites[sMonSummaryScreen->moveCategoriesSpriteId].invisible = TRUE;
    ScheduleBgCopyTilemapToVram(0);
    HandlePowerAccTilemap(0, 3);
    HandleAppealJamTilemap(0, 3, 0);
...
static void PrintMoveDetails(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            if (B_SHOW_MOVE_CATEGORY_ICON == TRUE)
                ShowMoveCategory(GetBattleMoveCategory(move));
